(function (global) {
  function CommodoreLogo(A) {
    ...
    const fillMap = Loader.loadTexture('res/paper.png');

    const cee = new Path({ directionSize, fillMap, fill: true });
    ...
    const upperBeak = new Path({ directionSize, fillMap, fill: true });
    ...
    const lowerBeak = new Path({ directionSize, fillMap, fill: true });
