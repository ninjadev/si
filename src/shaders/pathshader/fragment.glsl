...
void main() {
    ...
    uv.y = uv.y * 4. - 2.;
    float originalCenteredY = uv.y;

    float humanizeX = uv.x * totalLength / 100. * 0.5;

    float offset = 0.75 * cnoise(vec3(humanizeX * 13., humanizeX * 17., humanizeX * 2. + id * 101.));
    ...
    float pencil = 0.5 + 1. * cnoise(99. * vec3(uv.x, uv.y * 10., 1.));
    amount *= pencil;
    amount *= pow(1. - sin(uv.y * PI - PI / 2.), 2.);
    amount = min(1., amount);
    amount = max(0., amount);
